...
namespace Applications\Dash\Packages\AdminLTETags;
...
class AdminLTETags extends BasePackage
{
    ...
    public function useTag(string $tagName, array $params)
    {
        ...
        if (count($explodedTagName) === 1) {
            $tag = 'Applications\\Dash\\Packages\\AdminLTETags\\Tags\\' . ucfirst($explodedTagName[0]);
        } else {
            $tag = 'Applications\\Dash\\Packages\\AdminLTETags\\Tags';
