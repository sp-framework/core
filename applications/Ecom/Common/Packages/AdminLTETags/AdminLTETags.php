...
namespace Applications\Ecom\Common\Packages\AdminLTETags;
...
class AdminLTETags extends BasePackage
{
    ...
    public function useTag(string $tagName, array $params)
    {
        ...
        if (count($explodedTagName) === 1) {
            $tag = 'Applications\\Ecom\\Common\\Packages\\AdminLTETags\\Tags\\' . ucfirst($explodedTagName[0]);
        } else {
            $tag = 'Applications\\Ecom\\Common\\Packages\\AdminLTETags\\Tags';
