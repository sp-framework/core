...
namespace Applications\Ecom\Dashboard\Packages\ABNLookup;
...
class ABNLookup extends BasePackage
{
    ...
    public function lookupABN(int $abn, string $historicalData = 'N')
    {
        ...
        if ($this->abn) {
            ...
            $supplierDetails = xmlToArray($response->getBody()->getContents(), 'string');

            $this->packagesData->supplierDetails = $supplierDetails['response'];
