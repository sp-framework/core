...
use Apps\Core\Packages\Devtools\Api\Contracts\ContractsOAPI;
use Apps\Core\Packages\Devtools\Api\Contracts\ContractsXML;
use Apps\Core\Packages\Devtools\Api\Contracts\Model\AppsCoreDevtoolsApiContracts;
...
{
    protected $modelToUse = AppsCoreDevtoolsApiContracts::class;
    ...
    protected function setContractDirectory($directory = null)
    {
        if ($directory) {
            ...
            //Default Directory
            $this->contractDirectory = 'apps/Core/Packages/Devtools/Api/Contracts/Contracts/';
        }
    ...
    protected function setServicesDirectory($category = null, $directory = null)
    {
        if (!$category && $directory) {
            ...
            if ($this->contract['type'] === 'system') {
                ...
                $this->servicesDirectory = 'apps/Core/Packages/System/Api/Apis/' . ucfirst($category) . '/';
            }
        }
    ...
    public function addContract(array $data)
    {
        ...
        if (isset($data['link']) && $data['link'] !== '') {
            ...
            try {
                $response = $this->remoteWebContent->request('GET', $data['link']);
