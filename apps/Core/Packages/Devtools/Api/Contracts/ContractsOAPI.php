...
namespace Apps\Core\Packages\Devtools\Api\Contracts;
...
class ContractsOAPI
{
    ...
    public function init($contract, $localContent)
    {
        ...
        if ($contract['type'] === 'system') {
            ...
            $this->servicesClass =
                'Apps\Core\Packages\System\Api\Apis\\' . ucfirst($this->contract['category']) . '\\';
    ...
    const HDR_XERO_TENANT_ID = \'xero-tenant-id\';';
        } else if ($this->contract['category'] === 'repos') {
            $const = '';
...
' . $headers . '
    ...
    protected function generateBaseServicesHeader()
    {
        ...
        $baseRestServiceNamespace = $this->servicesClass . ucfirst($this->contract['category']) . 'RESTService';
        $baseRestService = ucfirst($this->contract['category']) . 'RESTService';
...
';
    ...
    public function buildTypesFile()
    {
        ...
        $definitions = [];
        if (isset($this->contract['content']['components']['definitions'])) {
            $definitions = $this->contract['content']['components']['definitions'];
        } else if (isset($this->contract['content']['definitions'])) {
            $definitions = $this->contract['content']['definitions'];
        }
        ...
        if (count($definitions) > 0) {
            foreach ($definitions as $typeKey => $type) {
                $this->writeTypesFileContent($typeKey, $this->buildTypesFileContent($typeKey, $type));
            }
        }
    }
    ...
    protected function buildTypesFileContent($typeKey, $type)
    {
        $categoryServiceNamespace =
            'System\Base\Providers\BasepackagesServiceProvider\Packages\Api\Base\Types\BaseType';
...
{';
        ...
        } else if (isset($type['schema']['$ref'])) {
            $propertyName = $this->getRefClass($type['schema']['$ref'], true);

            $propertyTypes[$propertyName] = [];
            $propertyTypes[$propertyName]['type'] = $this->getRefClass($type['schema']['$ref']);
            if (isset($type['schema']['type']) && $type['schema']['type'] === 'array') {
                $propertyTypes[$propertyName]['repeatable'] = true;
            } else {
                $propertyTypes[$propertyName]['repeatable'] = false;
            }
            $propertyTypes[$propertyName]['attribute'] = false;
            $propertyTypes[$propertyName]['elementName'] = $propertyName;
        } else if (isset($type['schema']['items']['$ref'])) {
            $propertyName = $this->getRefClass($type['schema']['items']['$ref'], true);

            $propertyTypes[$propertyName] = [];
            $propertyTypes[$propertyName]['type'] = $this->getRefClass($type['schema']['items']['$ref']);
            if (isset($type['schema']['type']) && $type['schema']['type'] === 'array') {
                $propertyTypes[$propertyName]['repeatable'] = true;
            } else {
                $propertyTypes[$propertyName]['repeatable'] = false;
                    ...
                    if ($methodKey === 'get' &&
                        ...
                        foreach ($method['parameters'] as $parameterKey => $parameter) {
                            if (isset($parameter['in']) &&
                                ...
                                } else if (isset($parameter['type'])) {
                                    if ($parameter['type'] === 'string' ||
                                        $parameter['type'] === 'integer' ||
                                        $parameter['type'] === 'boolean'
                                    ) {
                                        $requestParams[$parameter['name']]['type'] = $parameter['type'];
                                        $requestParams[$parameter['name']]['repeatable'] = false;
...
namespace ' . $servicesNamespace . '\\' . ucfirst($this->contract['provider_name']) . ';
...
class ' . ucfirst($this->contract['provider_name']) . '
{
    ...
    public function init($apiConfig, $apiPackage)
    {
        $this->apiConfig = $apiConfig;

        $this->apiPackage = $apiPackage;

        $this->apiConfig[\'debug\'] = $this->debug;
    ...
    public function useService($serviceName, $serviceRequestParams = [])
    {
        try {
            $serviceClass =
                "' . $servicesClass . '\\\{$serviceName}\\\Services\\\{$serviceName}Service";
...
class ' . ucfirst($this->contract['category']) . 'RESTService extends BaseRESTService
{
    ...
    public static function getConfigDefinitions()
    {
        return [
            \'debug\'             => [
                \'valid\'             => [\'bool\', \'array\'],
                \'fn\'                => \'System\Base\Providers\BasepackagesServiceProvider\Packages\Api\Base\BaseFunctions::applyDebug\',
                \'default\'           => false
            ],
            \'authorization\'     => [
                \'valid\'             => [\'string\'],
                \'default\'           => self::$config[\'token\'],
                \'required\'          => true
            ],
            \'httpHandler\'       => [
                \'valid\'             => [\'callable\'],
                \'default\'           => \'System\Base\Providers\BasepackagesServiceProvider\Packages\Api\Base\BaseFunctions::defaultHttpHandler\'
            ],
            \'httpOptions\'       => [
                \'valid\'             => [\'array\'],
                \'default\'           => [
    ...
    protected function getRepoHeaders()
    {
        ...
        // Add required headers first.
        $headers[self::HDR_AUTHORIZATION] = \'token \' . $this->getConfig(\'token\');
