            ...
            foreach ($apiClientServices as $api) {
                ...
                if (count($api['used_by']) === 0) {
                    if (in_array($api['id'], $package['settings']['api_clients'])) {
                        $api['used_by'] = ['devtoolsmigrator'];
                        $api['in_use'] = 1;
                    }
                } else {
                    if (!in_array($api['id'], $package['settings']['api_clients'])) {
                        if (in_array('devtoolsmigrator', $api['used_by'])) {
                            $key = array_search('devtoolsmigrator', $api['used_by']);
                        ...
                        if (!in_array('devtoolsmigrator', $api['used_by'])) {
                            array_push($api['used_by'], 'devtoolsmigrator');
                            $api['in_use'] = 1;
                        }
                    }
                }
