...
class Middleware extends BasePackage
{
	public function run($postData)
	{
		...
		if ($this->modules->middlewares
				->getNamedMiddlewareForApp($postData['middlewareName'], $postData['app_id'])
		) {
			$this->packagesData->responseCode = 1;

			$this->packagesData->responseMessage =
				'Package ' . ucfirst($postData['middlewareName']) . ' already exists for this app.' .
		...
		if (!ctype_alpha($postData['middlewareName'])) {
			...
			$newMiddleware =
				$register->registerModule('middlewares', $postData['app_id'], $installedFiles, $names, $postData);
