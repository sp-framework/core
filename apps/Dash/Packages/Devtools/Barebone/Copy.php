...
class Copy extends BasePackage
{
	...
	public function copyModuleStructure($type, $names, $postData)
	{
		if (!$this->scanDir) {
			$this->scanDir =
				$this->localContent->listContents(
					'apps/Admin/Packages/Barebone/Data/',
					true
				);
		}
		...
		foreach ($contents as $contentKey => $content) {
			$destDir =
				str_replace('Barebone', $names['appName'],
					str_replace(
						'apps/Admin/Packages/Barebone/Data/',
						'',
						$content['path']
			...
			if ($content['basename'] === 'Barebone') {
				$content['basename'] = $names['appName'];
			}
	...
	protected function getTaskContentsFromScannedDir($type, $postData)
	{
		...
		if ($type === 'apps') {
			foreach ($this->scanDir as $key => $value) {
				if ($value['path'] === 'apps/Admin/Packages/Barebone/Data/apps/Barebone' ||
					$value['path'] === 'apps/Admin/Packages/Barebone/Data/apps/Barebone/app.json'
				) {
					$contents[$key] = $value;
				}
			}
