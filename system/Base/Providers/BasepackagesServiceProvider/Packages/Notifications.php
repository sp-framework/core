...
class Notifications extends BasePackage
{
    ...
    {
        // $notificationsSettings = $this->basepackages->settings->getNamedSettings('Notifications');

        // if ($notificationsSettings) {
        //     $this->notificationsSettings = Json::decode($notificationsSettings['settings'], true);

        //     if (!isset($this->notificationsSettings['noteTypes'])) {
        //         $this->notificationsSettings['noteTypes'] = $this->setNotificationsTypes;
        //     }
        // } else {
        //     $newSettings['package_name'] = 'Notifications';
        //     $newSettings['settings'] = [];
        //     $newSettings['settings']['noteTypes'] = $this->setNotificationsTypes();
        ...
        //     $this->notificationsSettings = $newSettings['settings'];
        // }
        ...
        if (isset($data['note_type']) && isset($data['note'])) {
            if ($data['note_type'] != 0 && $data['note_type'] != '') {
                ...
                if ($this->add($note)) {
                    ...
                    $this->packagesData->responseMessage = 'Notification Added';
                } else {
                    $this->packagesData->responseCode = 1;

                    $this->packagesData->responseMessage = 'Error Adding Notification';
                }
            }
        } else {
            $this->packagesData->responseCode = 1;

            $this->packagesData->responseMessage = "Notifications parameters missing";

            return;
        }
    }

    public function getNotifications($packageName, int $packageRowId, bool $newFirst = true)
    {
        $notificationsArr = $this->getByParams(
            [
        ...
        if ($notificationsArr && count($notificationsArr) > 0) {
            foreach ($notificationsArr as $key => &$note) {
                unset($note['id']);
            ...
            if ($newFirst && count($notificationsArr) > 1) {
                return array_reverse($notificationsArr);
            }
